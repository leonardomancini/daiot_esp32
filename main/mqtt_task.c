...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
            case MQTT_EVENT_CONNECTED:
                ...
                esp_mqtt_client_subscribe(event->client, "/projects/daiot-smart-home/topics/leds", 0);
            ...
            case MQTT_EVENT_DATA:
                ...
                if(root != NULL) {
                    ...
                    char *ledValue = led->valuestring;
                    printf("DATA=%.*s\r\n", event->data_len, ledValue);
...
void publicar_temperatura_task(void * parm)
{
	...
	char buffer_humi_txt[15];
    char buffer_rssi_txt[15];
	char buffer_led_state[15];
	...
	while(1)
	{
        if (mqtt_client_connected) {
            ...
            snprintf(buffer_humi_txt, sizeof(buffer_humi_txt), "%.2f", humi);
            snprintf(buffer_rssi_txt, sizeof(buffer_rssi_txt), "%d", rssi);
            snprintf(buffer_led_state, sizeof(buffer_led_state), "%d", led_state);
            ...
            fprintf(stdout, "%u\n", (int)time(NULL)); 
                ...
                strcat(bufferJson, buffer_humi_txt);
                 strcat(bufferJson, ", \"led_1\": ");
                strcat(bufferJson, buffer_led_state);
                strcat(bufferJson, " }");
